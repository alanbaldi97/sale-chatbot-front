...
const onConfirm = async () => {
    try {
        loading.value = true;
        await useFetchRequest(`/api/products/${props.item.id}`, {
            method: 'DELETE',
        });

        toast.add({
            title: $t('common.success'),
            description: $t('products.messages.deleteSuccess', { name: props.item.name }),
        ...
        toast.add({
            ...
            description: $t('products.errors.deleteFailed', { name: props.item.name }),
